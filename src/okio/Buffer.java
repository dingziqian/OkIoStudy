...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable, ByteChannel {
  ...
  public final long completeSegmentByteCount() {
    ...
    if (tail.limit < Segment.SIZE && tail.owner) {
      // 总的数量，减去最后一个segment的数量，就是前几个凑整的segment数量
  ...
  @Override public void readFully(byte[] sink) throws EOFException {
    int offset = 0;
    while (offset < sink.length) { // 这里会循环将sink写满
  ...
  /**
   * 将Source里面的流的全部数据写入this(buffer)中，并为buffer里面的数据添加segment
   * @param source
   * @return
   * @throws IOException
   */
  ...
  Segment writableSegment(int minimumCapacity) {
    ...
    if (head == null) { // head为空的情况，直接从缓存中取
  ...
  @Override public void write(Buffer source, long byteCount) {
    ...
    //
    // 复制大量数据非常昂贵。 相反，我们宁愿将整个片段从一个缓冲区重新分配给另一个缓冲区。
    // Copying large amounts of data is expensive. Instead, we prefer to
    // reassign entire segments from one buffer to the other.
    //
    // Don't waste memory.
    //
    // 作为一个不变量,除了head segment 和 tail segment，缓冲区中相邻的段对应至少有50％满
    //
    // As an invariant, adjacent pairs of segments in a buffer should be at
    // least 50% full, except for the head segment and the tail segment.
    //
    // head segment 无法保持不变,因为应用程序正在消耗来自该segment的字节，从而降低其级别
    // The head segment cannot maintain the invariant because the application is
    // consuming bytes from this segment, decreasing its level.
    //
    // 由于应用程序正在生成字节，tail segment 无法保持不变量，因此可能需要附加新的几乎为空的 tail segments
    ...
    //
    // 当将一个缓冲区写入另一个缓冲区时,我们宁愿重新分配整个segments，将字节复制到最紧凑的形式。
    // 假设我们有一个[91％，61％]segment的buffer。 如果我们再添加一个[72％]Segment的buffer，
    // 则会产生[91％，61％，72％]字节。 没有字节被复制。
    //
